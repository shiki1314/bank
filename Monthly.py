...
if __name__ == "__main__":
    ...
    for i,s in enumerate(["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"]):
      sqlcmd="select branch_name,sum(payment.amount) from payment,loan \
                  where payment.loan_id=loan.loan_id and payment_date like '2020-{}%' group by branch_name;".format(s)
    ...
    print(month)
    for i, s in enumerate(["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"]):
      sqlcmd = "select branch_name,sum(balance) from saving_account where open_date like '2020-{}%' group by branch_name;".format(
        s)
      # 利用pandas 模块导入mysql数据
      a = pd.read_sql(sqlcmd, dbconn)
      # if not a.values.any() :
