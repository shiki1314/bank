...
if __name__ == "__main__":
    ...
    for i,s in enumerate(["2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020"]):
      sqlcmd="select branch_name,sum(payment.amount) from payment,loan \
                  where payment.loan_id=loan.loan_id and payment_date like '{}-%' group by branch_name;".format(s)
      ...
      if a.values.any():
        for data in a.values:
          ...
          month[dict[data[0]]][i] = [i+2009,data[1]]
          # print(month)
    ...
    tableWidget.setVerticalHeaderLabels(['2009', '2010', '2011', '2012', '2013','2014','2015','2016','2017','2018','2019','2020'])
    for i in range(12):
      print(month[0][i][1])
    ...
    for i, s in enumerate(["2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020"]):
      sqlcmd = "select branch_name,count(borrow.customer_id) from payment,borrow,loan \
            where borrow.loan_id=loan.loan_id and payment.loan_id = loan.loan_id and payment.payment_date like '{}-%' group by branch_name;".format(
        s)
      # 利用pandas 模块导入mysql数据
      a = pd.read_sql(sqlcmd, dbconn)
      # if not a.values.any() :
    ...
    tableWidget_client.setRowCount(12)
    tableWidget_client.setColumnCount(2)  # 12行2列

    tableWidget_client.setHorizontalHeaderLabels(['合肥支行', '南京支行'])
    tableWidget_client.setVerticalHeaderLabels(['2009', '2010', '2011', '2012', '2013','2014','2015','2016','2017','2018','2019','2020'])
    for i in range(12):
      print(month[0][i][1])
    ...
    print(month)
    for i, s in enumerate(["2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020"]):
      sqlcmd = "select branch_name,sum(balance) from saving_account where open_date like '{}-%' group by branch_name;".format(
        s)
      # 利用pandas 模块导入mysql数据
      a = pd.read_sql(sqlcmd, dbconn)
      # if not a.values.any() :
    ...
    tableWidget_saving.setRowCount(12)
    tableWidget_saving.setColumnCount(2)  # 12行2列

    tableWidget_saving.setHorizontalHeaderLabels(['合肥支行', '南京支行'])
    tableWidget_saving.setVerticalHeaderLabels(['2009', '2010', '2011', '2012', '2013','2014','2015','2016','2017','2018','2019','2020'])
    for i in range(12):
      print(month[0][i][1])
    ...
    for i, s in enumerate(["2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018", "2019", "2020"]):
      sqlcmd = "select branch_name,count(depositor.customer_id) from depositor,saving_account \
                where depositor.account_id =saving_account.account_id and saving_account.open_date like '{}-%' group by branch_name;".format(
        s)
      # 利用pandas 模块导入mysql数据
      a = pd.read_sql(sqlcmd, dbconn)
      # if not a.values.any() :
    ...
    tableWidget_saving_client.setRowCount(12)
    tableWidget_saving_client.setColumnCount(2)  # 12行2列

    tableWidget_saving_client.setHorizontalHeaderLabels(['合肥支行', '南京支行'])
    tableWidget_saving_client.setVerticalHeaderLabels(['2009', '2010', '2011', '2012', '2013','2014','2015','2016','2017','2018','2019','2020'])
    for i in range(12):
      print(month[0][i][1])
